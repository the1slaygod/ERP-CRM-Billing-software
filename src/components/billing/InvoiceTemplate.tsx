    ...
    <div className="bg-white p-10 mx-auto w-full max-w-4xl text-zinc-800 font-sans print:p-0 print:max-w-full print:m-0">
      ...
      <div className="flex justify-between items-start mb-12">
        ...
        <div className="text-right">
          <h2 className="text-3xl font-bold text-zinc-800 tracking-tight uppercase">Invoice</h2>
          <div className="mt-4 space-y-1 text-sm text-zinc-500">
            <p><span className="font-medium text-zinc-700">Invoice No:</span> {invoice.number}</p>
            <p><span className="font-medium text-zinc-700">Date:</span> {new Intl.DateTimeFormat('en-US', { day: '2-digit', month: 'short', year: 'numeric' }).format(new Date(invoice.date))}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6 mb-12">
        ...
        <div className="border border-dashed border-zinc-300 rounded p-5">
          ...
          <div className="space-y-1 text-sm text-zinc-500">
            <p className="font-semibold text-zinc-700">{invoice.customer.name}</p>
            <p className="whitespace-pre-wrap leading-relaxed">{invoice.customer.billingAddress || "Client Address not provided"}</p>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-8">
        ...
        <div className="w-80 flex-none">
          <div className="border-y border-zinc-300 py-2 mb-4">
            <h3 className="text-center font-semibold text-zinc-700 tracking-wide uppercase text-sm">Invoice Summary</h3>
          </div>
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between items-center pt-4 border-t border-zinc-200 mt-2">
              ...
              <span className="text-xl font-bold text-blue-600">₹{invoice.grandTotal.toFixed(2)}</span>
            </div>
          </div>
        </div>
      </div>
