...
type Product = { id: string; name: string; sku: string; sellingPrice: number; gstPercent: number; hsnSac: string | null; unit: string };
...
export function SalesOrderForm({ customers, products }: { customers: Customer[], products: Product[] }) {
  ...
  const calculatedLines = lines.map(line => {
    ...
    const taxAmount = (taxableValue * line.gstPercent) / 100;
    const cgst = taxAmount / 2;
    const sgst = taxAmount / 2;
    const total = taxableValue + taxAmount;
    
    return { ...line, taxableValue, cgst, sgst, igst: 0, total };
  });
  ...
  const handleAddLine = () => setLines([...lines, { productId: "", quantity: 1, rate: 0, discount: 0, gstPercent: 0, unit: "pcs" }]);

  const handleProductSelect = (index: number, productId: string) => {
    const product = products.find(p => p.id === productId);
    if (!product) return;
    const newLines = [...lines];
    newLines[index] = { ...newLines[index], productId, rate: product.sellingPrice, gstPercent: product.gstPercent, unit: product.unit };
    setLines(newLines);
  };
  ...
  const handleSave = async () => {
    ...
    const result = await createSalesOrder(customerId, calculatedLines, subtotal, totalDiscount, totalTaxable, totalCgst, totalSgst, 0, 0, grandTotal);
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-20">
      <div className="flex items-center justify-between">
        ...
        >
          {isSaving ? "Saving..." : "Confirm Sales Order"}
        </button>
      </div>
