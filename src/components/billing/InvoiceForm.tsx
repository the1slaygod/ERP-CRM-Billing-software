...
export function InvoiceForm({ customers, products }: { customers: Customer[], products: Product[] }) {
  ...
  const calculatedLines = lines.map(line => {
    ...
    // Assuming intra-state (CGST + SGST) for simplicity, could be derived from Customer State
  ...
  const handleAddLine = () => {
    setLines([...lines, { productId: "", quantity: 1, rate: 0, discount: 0, defaultGstPercent: 0, productCategory: null, unit: "pcs" }]);
  };
  ...
  const handleProductSelect = (index: number, productId: string) => {
    ...
    newLines[index] = {
      ...
      defaultGstPercent: product.gstPercent,
  ...
  const handleSave = () => {
    if (!customerId || lines.length === 0 || lines.some(l => !l.productId)) {
      toast.error("Please select a customer and ensure all items have a product selected.");
      return;
    }
          ...
          <Card>
            ...
            <CardContent>
              <select 
                ...
                className="w-full rounded-md border-0 py-2 pl-3 pr-10 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-indigo-600 sm:text-sm bg-white"
              >
              ...
              {selectedCustomer && (
                <div className="mt-4 p-4 bg-slate-50 rounded-md border text-sm text-slate-700">
                  ...
                  {selectedCustomer.gstin && <div className="mt-2 font-mono text-xs">GSTIN: {selectedCustomer.gstin}</div>}
                </div>
              )}
            </CardContent>
          </Card>
                  ...
                  {lines.map((line, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex gap-4 items-start p-4 bg-slate-50 rounded-md border">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <select 
                                ...
                                className="mt-1 w-full rounded-md border-0 py-1.5 pl-3 pr-10 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-indigo-600 sm:text-sm bg-white"
                              >
                        ...
                        <button onClick={() => removeLine(idx)} className="p-2 text-red-400 hover:text-red-600 bg-white rounded shadow-sm border mt-6">
        ...
        <div className="space-y-6">
          <Card className="sticky top-6">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex justify-between text-sm font-medium text-slate-900 pt-2 border-t">
              ...
              <div className="flex justify-between text-lg font-bold text-indigo-700 pt-4 border-t mt-4">
